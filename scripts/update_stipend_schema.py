...
def migrate_stipend_schema():
    print("Starting migration of stipend field...")
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        for position in positions:
            # Convert stipend to text
            stipend = position[7]
            if stipend is not None:
                try:
                    # Try to convert to float first
                    stipend = f"{float(stipend):.2f}"
